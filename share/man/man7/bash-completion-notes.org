...
=$prefix/bin= is in =PATH=.  This means we would put the files in the same
location but we would not need to add =$prefix/share= to =XDG_DATA_DIRS= as long
as we are adding =$prefix/bin= to the =PATH= which we are probably already doing.
