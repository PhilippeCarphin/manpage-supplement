...
looks for files whose name matches
...
in a list of directories formed of the following four groups (see CODE below
for code references):
...
In versions 2.12+ include =$d/share/bash-completion/completions= for each 
=d= in
- the dirname of the command if command contains a =/=,
- the dirname of the realpath of command if command contains a =/=
- =${d%/*}/share/bash-completion/completions= for all =$d= in =PATH= that
  end with =/bin= or =/sbin= (~d=$prefix/bin~ becomes
  =$prefix/share/bash-completion/completions=).

The first two mean that if an executable is called with a relative path or an 
absolute path, then bash completion will look in the same directory as the one
containing the executable.

Note also that directories in =PATH= that do not end in =/bin= or =/sbin= are
...
git will
- Call the appropriate completion sub-function if it exitsts (for example,
  =git commit= will be completed by code in =git-completion.bash=,
...
- Call =_completion_loader git-my-command= which will look for a file named
  =[_]git-my-command[.bash]= in the directories described above and should
  define the shell function =_git_my_command=.
- Reattempt to call the shell function =_git_my_command=.

Note that in this case, we have less freedom in how we write our file.

In the general case, lazy loading looks for a file which is expected to
define a compspec for =COMMAND=.  This compspec can set any function as the
completion function for =COMMAND=.

On the other hand, although =git= completion uses =bash-completion= to find
and source a file defining a completion for =git-my-command=, after this
happens, it directly attemps to call a function named =_git_my_command=.

Also note that the name of the function has any dashes in the command name
replaced by underscores (see *Git* subsection of *CODE* below).
...
In both of the following cases, it will skip everything until the =ls-files=
#+begin_src shell
[alias]
    x = ls-files
    y = "!f(){ : git ls-files ; git ... ; } ; f"
#+end_src
therefore =git x ...= and =git y ...= will have the same completion as =git
ls-files=.

Note that =ls-files= could also not be a builtin command and in that case,
the procedure described in *Files named =git-my-command=* will also be done.
